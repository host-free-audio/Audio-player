...
- **iPhone (Safari):** tap the Share icon → "Add to Home Screen" to get an app-like icon that opens without browser chrome.
- **Android (Chrome):** tap the ⋮ menu → "Add to Home screen" for the same effect.
- Playback continues in the background and shows lock-screen/notification controls (play, pause, skip) on most modern mobile browsers.
...
```
`artist` and `title` are optional per entry — omit them and the app guesses from the filename. Just make sure `"file"` matches the filename in `songs/` exactly, and there's a comma between entries but not after the last one.
